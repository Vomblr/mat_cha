...
class Account:
	...
	@classmethod
	def get_all_users(cls, user_match, filters=None, sort_by=None):# аргументы(ссылка на текущий класс, ссылка на текущего юзера, установленные фильтры, )
		sql = ("SELECT id FROM `users` "
			"WHERE NOT (biography is NULL OR age IS NULL OR city IS NULL OR gender IS NULL "
			"OR preferences IS NULL)")
		if user_match:
			sql, values = cls.filter_by_preferences(sql, user_match)# возвращает дополненный sql запрос и массив предпочтений (пол, предпочтение)
			users = db.get_all_rows(sql, values=values)# возвращает массив подходящих юзеров по предпочтениям
		else:
			users = db.get_all_rows(sql) # возвращает всех юзеров, у которых заполнены основные данные
		users = [Account(user, extended=False) for user in users] # создает сущности юзеров посредством конструктора аккаунт
		fame_rates = cls.get_fame_rating() # возвращает список с рейтингом пользователей
		tags_groups = cls.get_tags() # возвращает список с тэгами
		for user in users:
			user.fame = fame_rates.get(user.id, 0) if fame_rates else 0 # назначает каждому польхователю его рейтинг
			user.tags = tags_groups.get(user.id, ()) if tags_groups else () # назначает каждому польхователю его теги
		if filters is not None:
			users = [user for user in users if user.filter_fit(filters)] # применяет фильтры с главной страницы (строго)
		sort_func = cls.create_sort_func(user_match, sort_by) # создает функцию обработки
		if sort_func is not None:
			users = sorted(users, key=sort_func) # сортирует юзеров по возвращаемому значению функции обработки
		return users #
	...
	def create_sort_func(user_match, sort_by=None):
		...
		def sort(e): # (принимает) юзеров которых ищем
			if sort_by:
				if sort_by == 'age' or sort_by == 'fame':
					return -getattr(e, sort_by) # возвращает отрицательное числовое значение (первыми в выдаче будут большие значения)
				elif user_match is not None and sort_by == 'common_tags':
					return -len(set(user_match.tags).intersection(e.tags)) # возвращает количество тэгов (после обработки первыми будут  юзеры с большим количеством тегов)
				elif user_match is not None and sort_by == 'city':
					return e.city != user_match.city # true or false совпадают ли города у юзера и остальных людей с сайта
				return (e.city != user_match.city, abs(user_match.age - e.age), -e.fame,
						-len(set(user_match.tags).intersection(e.tags))) # если нет параметров сортировки, тогда сортирует сразу по всем параметрам (город->возраст->рейтинг->кол-во тэгов)
