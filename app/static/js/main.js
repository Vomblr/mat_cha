		...
		notificationsVue = new Vue({
			...
			created() {
				...
				this.socket.on('notification received', response => {
					...
					this.sendNotification('У вас новое сообщение!', null);
				});
			},
			methods: {
				...
				sendNotification(title, options) {
					...
					// Проверим, есть ли права на отправку уведомлений
						...
						function clickFunc() {
							window.location.href = "https://matcha.mcdir.ru/";
						}
	...
	if ((city = document.getElementById('city')) && city.value === '') {
		...
		let geoSuccess = function(position) {
			axios.get("https://eu1.locationiq.com/v1/reverse.php", {
				params: {
					key: "pk.6688b778e79e5a1cbf08e9e106ce540b",
				...
				let res = response.data["display_name"];
				if (res) {
					let splitted = res.split(', ');
					city.value = splitted[splitted.length - 4] + ", " + splitted[splitted.length - 1];
				}
			});
		};
